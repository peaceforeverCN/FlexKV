#!/usr/bin/env python3
"""Multi-layer layerwise benchmark: SDMA vs compute kernel.
...
def bench_multi_layer(num_blocks, num_layers, iters, is_h2d, chunk_size,
    ...
    ce_kt = kernel_threshold if mode == "auto" else 0
    ...
    kwargs = dict(
        ...
        ce_segment_threshold=8, ce_force_path=force_path,
...
def main():
    ...
    for mode in ["sdma", "compute_kernel"]:
        name = {"sdma": "SDMA", "compute_kernel": "COMPUTE_KERNEL"}[mode]
        print(f"\n{'='*75}")
        print(f"  Mode: {name}")
        print(f"{'='*75}")
        ...
        for nb, nl, label in cases:
            h2d_med, _ = bench_multi_layer(nb, nl, args.iters, True, 1024, mode, 0)
            d2h_med, _ = bench_multi_layer(nb, nl, args.iters, False, 1024, mode, 0)
